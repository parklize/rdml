...
if __name__=='__main__':
    ...
    for i in range(config.networks):
        models.append(fastText(config, len(dataset.vocab), dataset.word_embeddings))
        models[-1].to(DEVICE)
    ...
    schedulers = [
        optim.lr_scheduler.StepLR(
            optimizer, step_size=config.lr_step_size, gamma=config.lr_decay, last_epoch=-1)
    ...
    for i in range(config.max_epochs):
        ...
        for j, val_acc in enumerate(val_accuracies):
            if val_acc >= best_val_acc[j]:
                ...
                print(f"{j+1}-th model didn't improve from {best_val_acc[j]}")
                patience[j] += 1
    ...
    for i in range(len(models)):
        ...
        with open(f'results/{fn}/best_results-{i+1}.txt', 'w') as f:
            for v in dir(config):
                if '__' not in v:
